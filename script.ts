...
program
  ...
  .option(
    "-pr, --prompt <prompt>",
    `additional details to provide to the prompt [ex: "(portrait), (toddler), (boy), ((frame from a Studio Ghibli movie))"]`,
    "(portrait), (extra detailed)"
  )
...
async function makeStory() {
  ...
  const {
    ...
    story: Array<{ paragraph: string; description: string }>;
  ...
  for (const [index, paragraph] of story.entries()) {
    const sdTxt2ImgResp = await fetch(
      "http://127.0.0.1:7860/sdapi/v1/txt2img",
      {
        ...
        body: JSON.stringify({
          prompt: `<lora:${lora}:${loraWeight}> ${paragraph.description}${
            prompt ? ` ${prompt}` : ""
          }`,
          negative_prompt:
    ...
    <body>
      <table>
        ${story
          .map(
            ({ paragraph }, index) =>
              `<tr><td><img src="./${index}.png" /></td><td><h1>${paragraph}</h1></td></tr>`
          )
          .join("")}
      </table>
    </body>
